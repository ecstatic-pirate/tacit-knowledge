import { NextRequest, NextResponse } from 'next/server'
import { createClient } from '@supabase/supabase-js'
import type { Database, Json } from '@/lib/supabase/database.types'

// HTML escape to prevent XSS
function escapeHtml(text: string | undefined | null): string {
  if (!text) return ''
  const map: Record<string, string> = {
    '&': '&amp;',
    '<': '&lt;',
    '>': '&gt;',
    '"': '&quot;',
    "'": '&#039;'
  }
  return text.replace(/[&<>"']/g, m => map[m])
}

export interface ParticipantSurveyData {
  areas_of_expertise?: string[];
  knowledge_to_capture?: string;
  questions_for_others?: string[];
  additional_notes?: string;
}

// Validate participant survey data
function validateSurveyData(data: unknown): ParticipantSurveyData | null {
  if (!data || typeof data !== 'object') return null

  const survey = data as Record<string, unknown>
  const result: ParticipantSurveyData = {}

  // Validate areas_of_expertise (array of strings, max 50 items)
  if (Array.isArray(survey.areas_of_expertise)) {
    result.areas_of_expertise = survey.areas_of_expertise
      .filter((q): q is string => typeof q === 'string')
      .slice(0, 50)
      .map(q => escapeHtml(q.slice(0, 500)))
  }

  // Validate knowledge_to_capture (string, max 10000 chars)
  if (typeof survey.knowledge_to_capture === 'string') {
    result.knowledge_to_capture = escapeHtml(survey.knowledge_to_capture.slice(0, 10000))
  }

  // Validate questions_for_others (array of strings, max 50 items)
  if (Array.isArray(survey.questions_for_others)) {
    result.questions_for_others = survey.questions_for_others
      .filter((q): q is string => typeof q === 'string')
      .slice(0, 50)
      .map(q => escapeHtml(q.slice(0, 500)))
  }

  // Validate additional_notes (string, max 5000 chars)
  if (typeof survey.additional_notes === 'string') {
    result.additional_notes = escapeHtml(survey.additional_notes.slice(0, 5000))
  }

  return result
}

// Create a Supabase client for public API access
function getServiceClient() {
  const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL!
  const supabaseKey = process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!
  return createClient<Database>(supabaseUrl, supabaseKey)
}

// GET: Validate token and fetch existing data
export async function GET(request: NextRequest) {
  const searchParams = request.nextUrl.searchParams
  const token = searchParams.get('token')

  if (!token) {
    return NextResponse.json({ error: 'Token is required' }, { status: 400 })
  }

  const supabase = getServiceClient()

  // Fetch token data with campaign and participant info
  const { data: tokenData, error: tokenError } = await supabase
    .from('campaign_access_tokens')
    .select(`
      *,
      campaigns (
        id,
        expert_name,
        expert_role,
        department,
        goal,
        subject_type,
        project_type
      )
    `)
    .eq('token', token)
    .eq('token_type', 'participant')
    .single()

  if (tokenError || !tokenData) {
    return NextResponse.json({ error: 'Invalid or expired token' }, { status: 404 })
  }

  // Check if token is expired
  if (new Date(tokenData.expires_at) < new Date()) {
    return NextResponse.json({ error: 'This link has expired' }, { status: 410 })
  }

  // Get the participant info based on the token's email/name
  const { data: participantData } = await supabase
    .from('participants')
    .select('*')
    .eq('campaign_id', tokenData.campaign_id)
    .eq('email', tokenData.email)
    .single()

  // Check for existing response
  let existingResponse = null
  if (participantData) {
    const { data: responseData } = await supabase
      .from('participant_survey_responses')
      .select('*')
      .eq('token_id', tokenData.id)
      .single()
    existingResponse = responseData
  }

  return NextResponse.json({
    tokenId: tokenData.id,
    campaignId: tokenData.campaign_id,
    participantEmail: tokenData.email,
    participantName: tokenData.name,
    participantRole: tokenData.role,
    submittedAt: tokenData.submitted_at,
    draftData: tokenData.draft_data,
    existingResponse: existingResponse ? {
      areas_of_expertise: existingResponse.areas_of_expertise,
      knowledge_to_capture: existingResponse.knowledge_to_capture,
      questions_for_others: existingResponse.questions_for_others,
      additional_notes: existingResponse.additional_notes,
    } : null,
    participant: participantData,
    campaign: tokenData.campaigns,
  })
}

// POST: Save draft or submit survey
export async function POST(request: NextRequest) {
  let body: unknown
  try {
    body = await request.json()
  } catch {
    return NextResponse.json({ error: 'Invalid JSON' }, { status: 400 })
  }

  const { token, data: rawData, submit } = body as {
    token: string
    data: unknown
    submit?: boolean
  }

  if (!token || typeof token !== 'string') {
    return NextResponse.json({ error: 'Token is required' }, { status: 400 })
  }

  // Validate and sanitize input data
  const data = validateSurveyData(rawData)
  if (!data) {
    return NextResponse.json({ error: 'Invalid survey data' }, { status: 400 })
  }

  const supabase = getServiceClient()

  // Verify token and get info
  const { data: tokenData, error: tokenError } = await supabase
    .from('campaign_access_tokens')
    .select('id, campaign_id, email, name, role, expires_at')
    .eq('token', token)
    .eq('token_type', 'participant')
    .single()

  if (tokenError || !tokenData) {
    return NextResponse.json({ error: 'Invalid token' }, { status: 404 })
  }

  if (new Date(tokenData.expires_at) < new Date()) {
    return NextResponse.json({ error: 'This link has expired' }, { status: 410 })
  }

  // Get or create participant
  let { data: participantData } = await supabase
    .from('participants')
    .select('id')
    .eq('campaign_id', tokenData.campaign_id)
    .eq('email', tokenData.email)
    .single()

  // If participant doesn't exist, create one
  if (!participantData) {
    const { data: newParticipant, error: createError } = await supabase
      .from('participants')
      .insert({
        campaign_id: tokenData.campaign_id,
        name: tokenData.name || 'Unknown',
        email: tokenData.email,
        role: tokenData.role || null,
        status: 'not_interviewed',
      })
      .select('id')
      .single()

    if (createError) {
      console.error('Error creating participant:', createError)
      return NextResponse.json({ error: 'Failed to create participant' }, { status: 500 })
    }
    participantData = newParticipant
  }

  // Update the token with draft data
  const tokenUpdate: { draft_data: Json; submitted_at?: string } = {
    draft_data: data as unknown as Json,
  }

  if (submit) {
    tokenUpdate.submitted_at = new Date().toISOString()
  }

  const { error: updateTokenError } = await supabase
    .from('campaign_access_tokens')
    .update(tokenUpdate)
    .eq('id', tokenData.id)

  if (updateTokenError) {
    console.error('Error updating token:', updateTokenError)
    return NextResponse.json({ error: 'Failed to save data' }, { status: 500 })
  }

  // If submitting, upsert the participant survey response
  if (submit && participantData) {
    // Check for existing response
    const { data: existingResponse } = await supabase
      .from('participant_survey_responses')
      .select('id')
      .eq('token_id', tokenData.id)
      .single()

    const responseData = {
      campaign_id: tokenData.campaign_id,
      participant_id: participantData.id,
      token_id: tokenData.id,
      areas_of_expertise: data.areas_of_expertise || [],
      knowledge_to_capture: data.knowledge_to_capture || null,
      questions_for_others: data.questions_for_others || [],
      additional_notes: data.additional_notes || null,
      submitted_at: new Date().toISOString(),
    }

    if (existingResponse) {
      const { error: updateError } = await supabase
        .from('participant_survey_responses')
        .update(responseData)
        .eq('id', existingResponse.id)

      if (updateError) {
        console.error('Error updating response:', updateError)
        return NextResponse.json({ error: 'Failed to save response' }, { status: 500 })
      }
    } else {
      const { error: insertError } = await supabase
        .from('participant_survey_responses')
        .insert(responseData)

      if (insertError) {
        console.error('Error inserting response:', insertError)
        return NextResponse.json({ error: 'Failed to save response' }, { status: 500 })
      }
    }
  }

  return NextResponse.json({
    success: true,
    submitted: submit ?? false,
  })
}
